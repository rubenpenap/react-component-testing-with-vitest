...
The last thing remaining is to adjust the assertions around the rendered elements on the page.

This is how my assertions looked like before:
...
Since I know that `.getBy*` methods now return _locators_, they **will always be truthy**. I need to assert on the elements, not locators. To do that, Vitest provides the `expect.element()` API:

```tsx filename=file-preview.test.tsx remove=1-2 add=3-4
expect(screen.getByText('file.txt')).toBeTruthy()
expect(screen.getByText('hello world')).toBeTruthy()
expect.element(screen.getByText('file.txt')).toBeTruthy()
expect.element(screen.getByText('hello world')).toBeTruthy()
```

Before, I was asserting on the HTML element being truthy, which, in itself, isn't a useful assertion to have because it's not written from the user's perspective.
